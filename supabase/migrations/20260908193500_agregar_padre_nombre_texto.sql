-- Padre como texto libre, además de animal.id_padre (que ya existía, pero
-- solo sirve cuando el padre es uno de los animales registrados en el
-- sistema). En la práctica, muchos padres son toros de campo/semen que
-- nunca se cargaron como animal -mismo patrón que ya existía para
-- evento_reproductivo, donde el toro de una inseminación queda anotado
-- como texto libre en el comentario ("Toro: Lautaro").
--
-- CREATE OR REPLACE VIEW solo permite agregar columnas al final.

set search_path to public;

alter table animal add column if not exists padre_nombre text;
...
values ('20260908193500_agregar_padre_nombre_texto')
on conflict (version) do nothing;

-- Comprobación: sigue una fila por animal, columna nueva presente.
select
    (select count(*) from v_animal_lista) as filas_vista,
    (select count(*) from animal) as animales;
select column_name, data_type from information_schema.columns
 where table_name = 'animal' and column_name = 'padre_nombre';
