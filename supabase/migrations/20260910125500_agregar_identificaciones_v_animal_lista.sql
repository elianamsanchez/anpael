-- Agrega `identificaciones` a v_animal_lista: todas las identificaciones
-- vigentes del animal concatenadas, no solo la principal que elige
-- v_ident_principal. Orden: caravana visual, marca a fuego, adicional, y
-- después RFID/SENASA si las tuviera -para no esconder ningún dato, pero
-- respetando el orden que se pidió para los tres tipos que importan en el
-- día a día.
--
...
values ('20260910125500_agregar_identificaciones_v_animal_lista')
on conflict (version) do nothing;

-- Comprobación: sigue una fila por animal, y aparecen las identificaciones
-- de un animal con más de una (ej. un toro con visual y fuego).
select
    (select count(*) from v_animal_lista) as filas_vista,
    (select count(*) from animal) as animales;
select id_animal, identificaciones from v_animal_lista where caravana = 'CO70';
-- esperado: "CO70 · 1122"
